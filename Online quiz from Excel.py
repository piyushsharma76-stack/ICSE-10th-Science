...
# ─────────────────────────────────────────────────────────────────────────────
# 3. MATH TEXT HANDLING
# ─────────────────────────────────────────────────────────────────────────────
# NOTE: Excel/.xlsx stores text as proper Unicode, so symbols typed directly
# into a cell — √, π, ², ³, ½, etc. — will now come through correctly on
# their own. You no longer need to "fake" them. The two things this section
# still fixes automatically are:
#   1. "^2", "^3", "^-1" style exponents  -> real superscript characters
#   2. "sqrt(x)" / "sqrt 9" typed as text -> √ symbol
#
# TIP for your Excel sheet: if you want to type √ or π directly, use
# Insert > Symbol (or copy/paste) rather than typing "sqrt" or "pi" as text,
# and it will show up as-is.
...
def _to_superscript(match):
    exponent = match.group(1)
    return ''.join(_SUPERSCRIPT_MAP.get(ch, ch) for ch in exponent)

def normalize_math_text(text):
    """Cleans up common plain-text math notation for nicer display."""
    if not isinstance(text, str):
        return text
    t = text

    # x^2, 5^3, x^-1  ->  x², 5³, x⁻¹
    t = re.sub(r'\^(-?\d+)', _to_superscript, t)

    # sqrt(9) / sqrt 9  ->  √(9) / √9   (only when someone typed the word)
    t = re.sub(r'\bsqrt\s*\(\s*([^)]+?)\s*\)', r'√(\1)', t, flags=re.IGNORECASE)
    t = re.sub(r'\bsqrt\s+(\d+(?:\.\d+)?)', r'√\1', t, flags=re.IGNORECASE)
...
def _clean_excel_cell(value):
    """
    Converts a raw openpyxl cell value into clean display text.

    Handles the classic Excel gotcha where typing a fraction like "1/2"
    or "3/4" gets silently auto-converted into a date. Excel's default
    fraction-to-date behaviour reads "D/M" input as month/day, so we
    reconstruct that as "day/month" text here. (If your sheet has real
    dates you care about, format those columns as Text/keep them out of
    option columns, since this reconstruction assumes any date found in
    a question/option cell was actually a mistyped fraction.)
    """
    if value is None:
        return ""
    ...
    if isinstance(value, float):
        # Whole numbers stored as float -> show as clean int
        if value.is_integer():
...
# 4. EXCEL LOADER
# ─────────────────────────────────────────────────────────────────────────────
EXCEL_FILENAME = "MCQ for test.xlsx"   # change this if your file has a different name
...
def load_data():
    ...
    try:
        ...
        ws = wb.active  # uses the first/active sheet in the workbook
        ...
        headers = [str(c.value).strip() if c.value is not None else f"Column{i+1}"
        ...
        for row in rows_iter:
            ...
            row_vals = [_clean_excel_cell(c.value) for c in row]
        ...
        if not data_rows:
            st.error("❌ No question rows found in the Excel file.")
            return pd.DataFrame()
    ...
    for col in df.columns:
        df[col] = df[col].apply(normalize_math_text)
...
# ── CHAPTER SELECT ────────────────────────────────────────────────────────────
    ...
    st.markdown("<h2 style='text-align:center;color:#FFD700;'>Select a Chapter</h2>",
                unsafe_allow_html=True)
    if df_all.empty:
        st.error("No data loaded. Please verify your Excel file path and structure.")
    else:
